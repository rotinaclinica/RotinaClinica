  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">Visão geral da plataforma</p>
      </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
          ...
          <div className="bg-white rounded-xl border border-zinc-200 p-4 border-l-4 border-l-emerald-400">
            <p className="text-xs text-zinc-600 dark:text-zinc-400 mb-1">Online agora</p>
            <p className="text-4xl font-bold text-zinc-900">{onlineNow}</p>
            <p className="text-[11px] text-zinc-600 dark:text-zinc-400 mt-0.5">usuários · janela de 5 min</p>
          </div>
          <div className="bg-white rounded-xl border border-zinc-200 p-4">
            <p className="text-xs text-zinc-600 dark:text-zinc-400 mb-1">Ativos — últimas 24h</p>
            <p className="text-4xl font-bold text-zinc-900">{active24h}</p>
            <p className="text-[11px] text-zinc-600 dark:text-zinc-400 mt-0.5">usuários únicos logados</p>
          </div>
          <div className="bg-white rounded-xl border border-zinc-200 p-4">
            <p className="text-xs text-zinc-600 dark:text-zinc-400 mb-1">Ativos — últimos 7 dias</p>
          ...
          <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b border-zinc-200">
                <tr>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400">Usuário</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400">Plano</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400">Vence em</th>
                </tr>
              </thead>
              <tbody>
                {renovandoLista.map((s, i) => (
                  <tr key={i} className="border-b border-zinc-50 last:border-0">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-zinc-600 dark:text-zinc-400">{s.user.email}</p>
                    </td>
      ...
      <section>
        ...
        {cancelamentosRecentes.length === 0 ? (
          <div className="bg-white rounded-xl border border-zinc-200 p-5 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b border-zinc-200">
                <tr>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400">Usuário</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400">Plano</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400">Cancelado em</th>
                </tr>
              </thead>
              <tbody>
...
function Tile({
  ...
  return (
    <div className={`bg-white rounded-xl border border-zinc-200 p-4 ${accent}`}>
      ...
      {sub && <p className="text-[11px] text-zinc-600 dark:text-zinc-400 mt-0.5">{sub}</p>}
    </div>
  );
}
