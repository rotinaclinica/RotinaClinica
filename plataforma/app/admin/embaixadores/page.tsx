    ...
    <div className="space-y-8">
      ...
      {ambassadors.length === 0 ? (
        <div className="bg-white rounded-xl border border-zinc-200 py-16 text-center">
          <p className="text-zinc-700 font-medium">Nenhum embaixador cadastrado ainda.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700">Embaixador</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700">Código</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700">Indicações</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700">Meses bônus</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700">Comissão pendente</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700">Já pago</th>
                  ...
                  <>
                    <tr key={amb.id} className="border-b border-zinc-100 hover:bg-zinc-50">
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-zinc-700">{amb.email}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-zinc-700">{paidCents > 0 ? brl(paidCents) : "—"}</td>
                      <td className="px-4 py-3">
                    ...
                    {amb.referralsGiven.length > 0 && amb.referralsGiven.map((r) => (
                      <tr key={r.id} className="border-b border-zinc-50 last:border-0 bg-zinc-50/60">
                        <td className="pl-10 pr-4 py-2" colSpan={2}>
                          <p className="text-xs text-zinc-700 font-medium">{r.referredUser.name ?? r.referredUser.email}</p>
                          <p className="text-[11px] text-zinc-700">{r.referredUser.email}</p>
                        </td>
                        <td className="px-4 py-2 text-xs text-zinc-700" colSpan={2}>
