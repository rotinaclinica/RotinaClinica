...
const statusStyle = (status: string) => {
  ...
  if (status === "EXPIRED")   return "bg-zinc-100 text-zinc-600 dark:text-zinc-400";
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <div>
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">Gerencie assinaturas manualmente</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-zinc-100 flex items-center justify-between">
          ...
          <span className="text-xs text-zinc-600 dark:text-zinc-400">{subscriptions.length} total</span>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-zinc-50 border-b border-zinc-200">
            <tr>
              <th className="text-left px-5 py-3 font-medium text-zinc-600 dark:text-zinc-400">Usuário</th>
              <th className="text-left px-5 py-3 font-medium text-zinc-600 dark:text-zinc-400">Plano</th>
              <th className="text-left px-5 py-3 font-medium text-zinc-600 dark:text-zinc-400">Status</th>
              <th className="text-left px-5 py-3 font-medium text-zinc-600 dark:text-zinc-400">Vence em</th>
          ...
          <tbody>
            {subscriptions.map((s) => (
              <tr key={s.id} className="border-b border-zinc-100 last:border-none">
                <td className="px-5 py-4">
                  ...
                  <div className="text-xs text-zinc-600 dark:text-zinc-400">{s.user.email}</div>
                </td>
            ...
            {subscriptions.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-zinc-600 dark:text-zinc-400">
