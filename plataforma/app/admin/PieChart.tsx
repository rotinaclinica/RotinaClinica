    ...
    <div className="bg-[#161b22] rounded-xl border border-white/10 p-4">
      ...
      <div className="flex flex-col items-center gap-3">
        <svg viewBox="0 0 120 120" className="w-20 h-20 flex-shrink-0">
          {paths.map((sl, i) => (
            <path key={i} d={sl.path} fill={sl.color} stroke="#161b22" strokeWidth="1.5" />
          ))}
          <circle cx="60" cy="60" r="28" fill="#161b22" />
          <text x="60" y="63" textAnchor="middle" fontSize="11" fontWeight="bold" fill="#e4e4e7">
            {total}
          </text>
        </svg>
        <div className="w-full flex flex-col gap-1.5">
          {slices.filter((sl) => sl.value > 0).map((sl, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-300 flex-1">{sl.label}</span>
