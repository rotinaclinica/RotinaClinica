...
const STATUS_LABEL: Record<string, { label: string; cls: string }> = {
  ...
  EXPIRED:   { label: "Expirado",  cls: "bg-zinc-100 text-zinc-700 dark:text-zinc-300" },
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead>
            <tr className="border-b border-zinc-100 bg-zinc-50">
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Usuário</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">CPF</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Celular</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Cadastro</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Último acesso</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Plano</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Status</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">1ª Compra</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Gasto total</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">Vence em</th>
          ...
          <tbody>
              ...
              return (
                <tr key={u.id} className="border-b border-zinc-50 last:border-0 hover:bg-zinc-50">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-zinc-700 dark:text-zinc-300">{u.email}</p>
                  </td>
                  <td className="px-4 py-3 text-xs">
                    {u.cpf ? (
                      <span className="text-zinc-700 dark:text-zinc-300 font-mono">{u.cpf.replace(/^(\d{3})(\d{3})(\d{3})(\d{2})$/, "$1.$2.$3-$4")}</span>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300 text-xs">{fmt(u.createdAt)}</td>
                  <td className="px-4 py-3 text-xs">
                    {u.lastSeenAt ? (
                      <span className="text-zinc-700 dark:text-zinc-300">{fmt(u.lastSeenAt)}</span>
                    ) : (
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={11} className="px-4 py-8 text-center text-zinc-700 dark:text-zinc-300">
