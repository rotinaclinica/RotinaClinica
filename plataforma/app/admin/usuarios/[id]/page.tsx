  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-700 dark:text-zinc-300 mt-0.5">{user.email}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-5">
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
          <div>
            <dt className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">CPF</dt>
          ...
          <div>
            <dt className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Celular</dt>
            <dd className="text-zinc-700 mt-0.5">{user.phone || <span className="text-zinc-300">—</span>}</dd>
          </div>
          <div>
            <dt className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Cadastro</dt>
            <dd className="text-zinc-700 mt-0.5">{fmt(user.createdAt)}</dd>
          </div>
          <div>
            <dt className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Último acesso</dt>
            <dd className="text-zinc-700 mt-0.5">{fmt(user.lastSeenAt)}</dd>
          </div>
          <div>
            <dt className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Gasto total</dt>
          ...
          <div>
            <dt className="text-xs text-zinc-700 dark:text-zinc-300 font-medium">Perfil</dt>
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-5">
        ...
        {user.orders.length === 0 ? (
          <p className="text-sm text-zinc-700 dark:text-zinc-300">Nenhum pedido.</p>
        ) : (
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-0.5 rounded uppercase ${
                      o.status === "PAID"
                        ...
                        : "bg-zinc-100 text-zinc-700 dark:text-zinc-300"
                    }`}
                  >
