...
import EbookCheckoutClient from "@/app/checkout/ebook/[slug]/EbookCheckoutClient";

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const product = await db.product.findUnique({ where: { slug, type: "COURSE", active: true }, select: { title: true } });
  return { title: product ? `${product.title} · Comprar` : "Curso não encontrado" };
}

export default async function CursoCheckoutPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;

  const product = await db.product.findUnique({
    where: { slug, type: "COURSE", active: true },
  ...
  if (!session?.user?.id) {
    redirect(`/login?callbackUrl=/checkout/curso/${slug}`);
  }
  ...
  if (alreadyOwns) redirect("/dashboard/cursos/destravando");

  const maxInstallments = product.priceCents >= 20000 ? 12 : product.priceCents >= 9000 ? 6 : product.priceCents >= 4000 ? 3 : 1;
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 md:py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            <div>
              <span className="inline-block bg-[#0f2d4a] text-white text-xs font-bold px-3 py-1 rounded-full mb-3">Curso Online</span>
            ...
            <div className="bg-gradient-to-br from-[#0f2d4a] to-[#1a4a6e] rounded-2xl p-5 text-white">
              ...
              <p className="text-xs text-white/80 leading-relaxed mb-3">
                Com a assinatura do Rotina Clínica, você tem acesso a este curso e todos os outros conteúdos,
                incluindo ebooks, prescrições, calculadoras, casos clínicos e muito mais
            ...
            <div className="bg-white dark:bg-[#131c2e] rounded-2xl border border-zinc-200 dark:border-white/8 p-5">
              ...
              <ul className="space-y-2">
                {[
                  "Acesso vitalício a todas as videoaulas",
                  "Material complementar para download",
                  "Abordagem prática das 10 queixas mais prevalentes",
          ...
          <div className="md:sticky md:top-8">
            ...
            <div className="flex items-center justify-center gap-4 mt-4 px-2">
              ...
              <span className="flex items-center gap-1.5 text-[11px] text-[#6a8fa5] font-medium">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#32bcad" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><circle cx="12" cy="12" r="10"/></svg>
                Acesso imediato
              </span>
              <span className="text-zinc-300 dark:text-zinc-600">|</span>
              <span className="flex items-center gap-1.5 text-[11px] text-[#6a8fa5] font-medium">
