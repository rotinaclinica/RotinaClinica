...
import {
  calcularDobutamina, SOLUCOES_DOBU,
  type DobutaminaInput, type DobutaminaResult, type SolucaoDobu,
} from "@/lib/calculadoras/dobutamina";

const COR: Record<DobutaminaResult["cor"], { badge: string; bar: string }> = {
...
export default function DobutaminaCalc() {
  const [vazao, setVazao]     = useState("");
  const [peso, setPeso]       = useState("");
  const [solucao, setSolucao] = useState<SolucaoDobu>("padrao");
  const [resultado, setResultado] = useState<DobutaminaResult | null>(null);
  ...
  function calcular() {
    ...
    setErro("");
    const input: DobutaminaInput = { vazao: v, peso: p, solucao };
    setResultado(calcularDobutamina(input));
  }

  function reset() { setResultado(null); setErro(""); }

  const cor = resultado ? COR[resultado.cor] : null;
  const barWidth = resultado ? Math.min(100, Math.round((resultado.dose / 20) * 100)) : 0;
  ...
  return (
    <div className="bg-white dark:bg-[#131c2e] rounded-2xl border border-zinc-200 dark:border-white/8 overflow-hidden">
      <div className="px-5 py-4 border-b border-zinc-100 dark:border-white/8 flex items-center gap-3">
        ...
        <div>
          <p className="font-bold text-sm text-[#0f2d4a] dark:text-[#e8edf5]">Dobutamina — Dose BIC</p>
          <p className="text-xs text-[#0f2d4a] dark:text-[#5a7a8e]">mcg/kg/min a partir da vazão em mL/h</p>
        </div>
      </div>

      <div className="p-5 space-y-4">
        <div>
          <p className="text-[11px] font-bold text-[#0f2d4a] dark:text-[#5a7a8e] uppercase tracking-widest mb-2">Tipo de solução</p>
          <div className="flex flex-col gap-1.5">
            {(["padrao", "concentrada"] as SolucaoDobu[]).map((s) => {
              ...
              return (
                <button key={s} onClick={() => { setSolucao(s); reset(); }}
                  ...
                  <span className="flex-1 font-semibold">{SOLUCOES_DOBU[s].label}</span>
                  <span className="text-[11px] text-[#0f2d4a] dark:text-[#5a7a8e]">{SOLUCOES_DOBU[s].descricao}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] text-[#0f2d4a] dark:text-[#8aacbc] mb-1 block">Vazão (mL/h)</label>
            <input type="number" value={vazao} placeholder="ex: 10"
          ...
          <div>
            <label className="text-[11px] text-[#0f2d4a] dark:text-[#8aacbc] mb-1 block">Peso (kg)</label>
        ...
        {resultado && cor && (
          <div className="rounded-xl border border-zinc-100 dark:border-white/8 bg-zinc-50 dark:bg-[#0f1e30] p-4 space-y-3">
            <div className="flex items-end gap-2">
              ...
              <p className="text-sm text-[#0f2d4a] dark:text-[#5a7a8e] mb-0.5">mcg/kg/min</p>
            </div>
            ...
            <p className="text-[11px] text-[#0f2d4a] dark:text-[#5a7a8e]">
              Dose inicial: 2,5–5 mcg/kg/min · Dose terapêutica: 5–20 mcg/kg/min
            </p>
          </div>
        )}
      </div>
