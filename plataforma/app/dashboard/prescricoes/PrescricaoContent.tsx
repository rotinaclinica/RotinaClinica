      ...
      {open && (
        ...
        >
          ...
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="fixed top-3 right-3 text-white/80 hover:text-white text-2xl font-light z-10 bg-black/60 rounded-full w-9 h-9 flex items-center justify-center"
          >
            ✕
          </button>
          <div className="absolute inset-0 overflow-auto" style={{ paddingTop: 52 }}>
            <div style={{ minHeight: "calc(100vh - 52px)", display: "flex", padding: 16 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={src}
                alt={caption ?? ""}
