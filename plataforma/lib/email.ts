...
export async function sendEbookSaleNotification({
  customerName,
  customerEmail,
  productTitle,
  valueCents,
  ...
  const valor = (valueCents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
  const gateway = paymentMethod === "pix" ? "PIX" : paymentMethod === "credit_card" ? "Cartão de crédito" : "Asaas";
  await resend.emails.send({
    from: FROM,
    to: "rotinaclinica77@gmail.com",
    subject: `Nova venda de ebook: ${productTitle}`,
    html: `
<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #dde6ef">
        <tr>
          <td style="background:#0f2d4a;padding:36px 40px">
            <p style="margin:0 0 4px;color:#3db8d4;font-size:12px;font-weight:600;letter-spacing:2px;text-transform:uppercase">Rotina Clínica — Admin</p>
            <p style="margin:0;color:#ffffff;font-size:22px;font-weight:700">Nova venda de ebook! 📚</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px">
            <p style="margin:0 0 20px;color:#4a6a80;font-size:15px;line-height:1.7">
              Um ebook acaba de ser vendido na plataforma. Confira os dados abaixo:
            </p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;border-radius:12px;margin-bottom:24px">
              <tr><td style="padding:20px 24px">
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>Produto:</strong> ${escapeHtml(productTitle)}</p>
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>Valor:</strong> ${valor}</p>
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>Cliente:</strong> ${escapeHtml(customerName)}</p>
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>E-mail:</strong> ${escapeHtml(customerEmail)}</p>
                <p style="margin:0;color:#0f2d4a;font-size:15px"><strong>Pagamento:</strong> ${gateway}</p>
              </td></tr>
            </table>
            <p style="margin:0;color:#94a8b8;font-size:13px;text-align:center">
              Acesse o <a href="https://rotina-clinica.vercel.app/admin/usuarios" style="color:#3db8d4;text-decoration:none">painel admin</a> para ver os detalhes.
            </p>
          </td>
        </tr>
        <tr>
          <td style="background:#f0f4f8;padding:20px 40px;text-align:center">
            <p style="margin:0;color:#94a8b8;font-size:12px">&copy; ${new Date().getFullYear()} Rotina Clínica</p>
          </td>
        </tr>
      </table>
    </td></tr>
...
export async function sendCancellationNotification({
  ...
  const data = accessUntil.toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });
  await resend.emails.send({
    from: FROM,
    to: "rotinaclinica77@gmail.com",
    subject: "Assinante cancelou a assinatura",
    html: `
<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f0f4f8;font-family:sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #dde6ef">
        <tr>
          <td style="background:#4a3000;padding:36px 40px">
            <p style="margin:0 0 4px;color:#f4c97a;font-size:12px;font-weight:600;letter-spacing:2px;text-transform:uppercase">Rotina Clínica — Admin</p>
            <p style="margin:0;color:#ffffff;font-size:22px;font-weight:700">Cancelamento de assinatura</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px">
            <p style="margin:0 0 20px;color:#4a6a80;font-size:15px;line-height:1.7">
              Um assinante cancelou a assinatura pela plataforma. O acesso permanece ativo até o fim do período pago.
            </p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;border-radius:12px;margin-bottom:24px">
              <tr><td style="padding:20px 24px">
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>Nome:</strong> ${escapeHtml(customerName)}</p>
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>E-mail:</strong> ${escapeHtml(customerEmail)}</p>
                <p style="margin:0;color:#0f2d4a;font-size:15px"><strong>Acesso ativo até:</strong> ${data}</p>
              </td></tr>
            </table>
            <p style="margin:0;color:#94a8b8;font-size:13px;text-align:center">
              Acesse o <a href="https://rotina-clinica.vercel.app/admin/usuarios" style="color:#3db8d4;text-decoration:none">painel admin</a> para ver os detalhes.
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #dde6ef">
        <tr>
          <td style="background:#7a1a1a;padding:36px 40px">
            <p style="margin:0 0 4px;color:#f4a0a0;font-size:12px;font-weight:600;letter-spacing:2px;text-transform:uppercase">Rotina Clínica — Admin</p>
            <p style="margin:0;color:#ffffff;font-size:22px;font-weight:700">Reembolso processado</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px">
            <p style="margin:0 0 20px;color:#4a6a80;font-size:15px;line-height:1.7">
              Um reembolso foi processado na plataforma Rotina Clínica. Confira os dados abaixo:
            </p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;border-radius:12px;margin-bottom:24px">
              <tr><td style="padding:20px 24px">
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>Nome:</strong> ${escapeHtml(customerName)}</p>
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>E-mail:</strong> ${escapeHtml(customerEmail)}</p>
                <p style="margin:0 0 10px;color:#0f2d4a;font-size:15px"><strong>Valor reembolsado:</strong> ${valor}</p>
                <p style="margin:0;color:#0f2d4a;font-size:15px"><strong>Gateway:</strong> ${gateway}</p>
              </td></tr>
            </table>
            <p style="margin:0;color:#94a8b8;font-size:13px;text-align:center">
              Acesse o <a href="https://rotina-clinica.vercel.app/admin/usuarios" style="color:#3db8d4;text-decoration:none">painel admin</a> para ver os detalhes do pedido.
            </p>
          </td>
        </tr>
        <tr>
