...
export async function processInvoiceBatch(limit = 20): Promise<InvoiceBatchResult> {
  ...
  for (const inv of pendentes) {
    ...
    try {
      ...
      //    Se já tem externalId (tentativa anterior criou a nota no Asaas mas
      //    falhou na autorização), pula direto pra consulta — não cria outra.
        ...
        if (emit.status === "error") {
          await db.invoice.update({
            ...
            data: {
              ...
              ...(inv.attempts + 1 >= MAX_ATTEMPTS ? { status: "FAILED" as const } : {}),
            },
          });
          if (inv.attempts + 1 >= MAX_ATTEMPTS) result.failed++;
          continue;
        }
        await db.invoice.update({
          where: { id: inv.id },
      ...
      } else if (inv.status === "PENDING" && inv.externalId) {
        externalRef = inv.externalId;
        await db.invoice.update({
          where: { id: inv.id },
      ...
      // 2. Consulta o status atual — se ainda processando, espera e tenta de novo.
      let consulta = await provider.consultar(externalRef ?? inv.providerRef);

      if (consulta.status === "processing") {
        await new Promise((r) => setTimeout(r, 25_000));
        consulta = await provider.consultar(externalRef ?? inv.providerRef);
      }
...
export async function retryInvoice(invoiceId: string): Promise<void> {
  ...
  try {
    ...
    if (consulta.status === "processing") {
      await new Promise((r) => setTimeout(r, 25_000));
      consulta = await provider.consultar(externalRef ?? uniqueRef);
    }
